...
impl<'a, A: Alarm<'a>> ST7735<'a, A> {
    ...
    fn send_command_with_default_parameters(&self, cmd: &'static Command) {
        ...
        self.buffer.map_or_else(
            || panic!("st7735: send parameters default has no buffer"),
            |buffer| {
                buffer[0] = cmd.id;
    ...
    fn send_command(&self, cmd: &'static Command, position: usize, len: usize, repeat: usize) {
        ...
        if let Some(dc) = self.dc {
            dc.clear();
                ...
                self.bus
                    .set_addr(BusWidth::Bits8, cmd.id as usize);
    ...
    fn do_next_op(&self) {
        // debug! ("{:?}, buffer, {}", self.status.get(), self.buffer.is_some());
...
impl<'a, A: Alarm<'a>> bus::Client for ST7735<'a, A> {
...
impl<'a, A: Alarm<'a>> spi::SpiMasterClient for ST7735<'a, A> {
    fn read_write_done(
        ...
        // debug!("read write done with status {:?}", self.status.get());
        if self.status.get() == Status::SendParametersSlice {
