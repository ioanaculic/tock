...
impl<'a> Fxos8700cq<'a> {
    ...
    fn start_read_accel(&self) {
        ...
        self.buffer.take().map(|buf| {
            ...
            self.i2c.write(buf, 3).expect ("i2c device error");
    ...
    fn start_read_magnetometer(&self) {
        self.buffer.take().map(|buf| {
            ...
            self.i2c.write(buf, 2).expect ("i2c device error");
...
impl gpio::Client for Fxos8700cq<'_> {
    fn fired(&self) {
        self.buffer.take().map(|buffer| {
            ...
            self.i2c.write_read(buffer, 1, 6).expect ("i2c device error"); // read 6 accel registers for xyz
    ...
    fn command_complete(&self, buffer: &'static mut [u8], _error: Error) {
        match self.state.get() {
            ...
            State::ReadMagStart => {
                ...
                self.i2c.write_read(buffer, 1, 6).expect ("i2c device error");
            }
            State::ReadMagValues => {
