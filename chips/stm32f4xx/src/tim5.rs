...
const TIM5_BASE: StaticRef<Tim5Registers> =
    unsafe { StaticRef::new(0x40000C00 as *const Tim5Registers) };

pub struct Tim5<'a> {
    registers: StaticRef<Tim5Registers>,
    clock: Tim5Clock<'a>,
...
impl<'a> Tim5<'a> {
    pub const fn new(rcc: &'a rcc::Rcc) -> Self {
        Self {
            registers: TIM5_BASE,
            clock: Tim5Clock(rcc::PeripheralClock::new(
                rcc::PeripheralClockType::APB1(rcc::PCLK1::TIM5),
                rcc,
            )),
    ...
    // starts the timer
    pub fn start(&self) {
        // TIM5 uses PCLK1. By default PCLK1 uses HSI running at 16Mhz.
        // Before calling set_alarm, we assume clock to TIM5 has been
        // enabled.
...
//         self.registers.dier.modify(DIER::CC1IE::SET); // asta activeaza intreruperea!!!
//     }
...
struct Tim5Clock<'a>(rcc::PeripheralClock<'a>);

impl ClockInterface for Tim5Clock<'_> {
